...
| guard retaliation and nearby guard assistance | Migrated and gameplay-verified | positive hate remains authoritative |
| select the currently most hated visible valid target | Migrated; gameplay retest required | deterministic higher/lower-hate tests; post-damage `ThreatChanged` wake |
...
| interrupt return when attacked/aggressed again | Migrated; gameplay retest required | return-state attack test |
...
| fresh acquisition range and line of sight | Migrated; gameplay retest required | 3D range in Brain plus authoritative Gateway revalidation |
...
| attack timeout/corpse camping policy | Audit pending | compare `_attackTimeout` and corpse configuration branches |
