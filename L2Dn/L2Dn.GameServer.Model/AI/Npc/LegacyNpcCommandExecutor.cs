...
internal sealed class LegacyNpcCommandExecutor: ILegacyNpcCommandExecutor
{
    ...
    public void StartFollow(AbstractAI ai, Creature target) =>
        Execute("start_follow", () => ai.startFollow(target));
    ...
    public void ReturnHome(Attackable npc) => Execute("return_home", npc.returnHome);
    ...
    public void AbortAttack(Creature actor) => Execute("abort_attack", actor.abortAttack);
    ...
    public void ClearCombatMemory(Attackable npc) => Execute("clear_combat_memory", () =>
    {
        npc.clearAggroList();
